...
class FileMeasurement(Measurement):
    ...
    def create_parser_object(self, type, name, columns=None):
        ...
        Returns:
            An file object
        """ 
        from efs_parser.utils import create_parser_object as cpo
    ...
    async def get_byteRanges(self, chr, start, end, bins, bin=True):
        result = None
        err = None
        
        logging.debug("File Measurement: %s\t%s\t%s" %(self.mid, self.name, "file_get_byteRanges"))

        print("in class get byte ranges")
        ...
        try:
            if self.fileHandler is None:
                ...
                result, err = await self.fileHandler.handleFileByteRanges(self.source, self.mtype, chr, start, end, bins=bins)
 
            return result, str(err)
        except Exception as e:
            logging.error("File Measurement: %s\t%s\t%s" %(self.mid, self.name, "file_get_byteRanges"), exc_info=True)
            return {}, str(e)
