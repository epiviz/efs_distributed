...
class FileHandlerProcess(object):
    ...
    @cached(ttl=None, cache=Cache.MEMORY, serializer=PickleSerializer(), namespace="handlefilebyteranges")    
    async def handleFileByteRanges(self, fileName, fileType, chr, start, end, bins = 2000):
        logging.debug("Handler: %s\t%s" %(fileName,  "handleFileByteRanges"))
        
        print("in handler get byte ranges")

        try:
            fileObj = await self.get_file_object(fileName, fileType)
            await self.client.wait_for_workers(1)
            data, err = await fileObj.getByteRanges(chr, start, end, bins)
            return data, err
        except Exception as e:
            #  a way of resilience
            logging.debug("Handler: Exception & resubmit %s\n%s\t%s" %(str(e), fileName,  "handleFileByteRanges"))
            if fileName in self.records:
                del self.records[fileName]
            return await self.handleFileByteRanges(fileName, fileType, chr, start, end, bins=bin)
